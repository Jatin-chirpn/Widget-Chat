...
export default function WidgetPanel(props) {
  const { adminId, currentUserId, inboxId } = useContext(Chat);
  ...
  async function sendMessage(event) {
    ...
    await updateDoc(doc(db, "userChats", admin), {
      [inboxId + ".lastMessage"]: {
        text,
      },
      [inbox + ".date"]: serverTimestamp(),
  ...
  return (
    <>
      <div
        className={`rounded-2xl shadow-2xl bottom-28 right-16 w-[92vw] h-full duration-700 ${props.className}`}
      >
        <Container>
          <div className="h-[10vh] flex bg-gradient-to-r from-[#0057FF] to-[#0057FF] pl-5  pt-5 rounded-2xl rounded-b-none text-center">
            <svg
        ...
        <Container className="bg-white rounded-b-none">
          <div className="min-h-full h-[54.6vh] overflow-y-auto py-2">
        ...
        <form
          ...
          className="h-[10vh] border-t-[0.4vh] bg-white flex items-center justify-around rounded-b-2xl"
        >
          <input
